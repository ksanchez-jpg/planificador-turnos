...
def generar_programacion_equitativa(n_ops, d_req, n_req, d_semana):
    ...
    for bloque_idx in [0, 21]:
        ...
        for d in bloque:
            ...
            # LÓGICA MÍNIMO 2 DÍAS: Identificar quién debe seguir trabajando hoy
            obligados = []
            for op in ops:
        ...
        # FASE 2: REFUERZOS (Buscando bloques de 2 para evitar días solos)
        for op in ops:
            intentos = 0
            while turnos_bloque[op] < 11 and intentos < 200:
                ...
                # Para evitar el "serrucho", preferimos días con vecinos trabajando
                tiene_vecino = False
...
if st.button(f"🚀 Generar Programación para {cargo}"):
    total_turnos_ciclo = (demanda_dia + demanda_noche) * demanda_dia * 3 # Ajuste de lógica de cálculo
