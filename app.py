...
st.caption("Modelo 2x2 por bloques, 132h garantizadas y Límite Estricto de 2 Refuerzos.")
...
# --- MOTOR DE CÁLCULO OPTIMIZADO PARA NÓMINAS GRANDES ---
...
def calcular_motor_final(n_ops, d_req, n_req, d_semana, seed):
    ...
    for bloque_idx in [0, 21]:
        ...
        # FASE 2: REFUERZOS Y GARANTÍA DE HORAS (Nivelación Estricta a 2)
...
if "df" in st.session_state:
    ...
    # SOLUCIÓN AL KEYERROR: Renombrar el índice primero y crear un objeto nuevo
    df_visual = df_base.copy()
    if st.session_state['mapping']:
        df_visual.index = [st.session_state['mapping'].get(x, x) for x in df_visual.index]
    
    c1, c2, c3 = st.columns(3)
    with c1: st.markdown(f'<div class="metric-box-green"><div>Personal Total</div><div class="metric-value-dark">{op_f}</div></div>', unsafe_allow_html=True)
    with c2: st.markdown(f'<div class="metric-box-green"><div>Fichas Reales</div><div class="metric-value-dark">{len(fichas_cargadas)}</div></div>', unsafe_allow_html=True)
    ...
    # Aplicamos el estilo sobre el DataFrame que ya tiene los nombres cambiados
    st.dataframe(df_visual.style.map(style_f), use_container_width=True)

    st.subheader("📊 Balance Detallado por Operador")
    stats = []
    for idx_orig in df_base.index:
        ...
        stats.append({
            "Identidad": identidad,
            "T. Día": int((f==TURNO_DIA).sum()), "T. Noche": int((f==TURNO_NOCHE).sum()),
    ...
    st.subheader("✅ Validación de Cobertura (Máximo 2 Refuerzos)")
    check = []
    ...
    # --- EXCEL PROFESIONAL CON COLORES Y BALANCE ---
    out = io.BytesIO()
    wb = Workbook()
