...
with st.sidebar:
    ...
    if archivo_subido:
        ...
        if not df_excel.empty:
            ...
            conteo_sugerido = len(fichas_cargadas) [cite: 2, 3]
...
# 6. RENDERIZADO Y EXPORTACIÓN MODIFICADA
if "df" in st.session_state:
    ...
    df_balance_final = pd.DataFrame(stats).set_index("Identidad")
    st.dataframe(df_balance_final, use_container_width=True)
    ...
    df_check_final = pd.DataFrame(check).set_index("Día")
    st.dataframe(df_check_final.T, use_container_width=True)

    # --- NUEVA LÓGICA DE DESCARGA ---
    out = io.BytesIO() [cite: 26]
    # Usamos xlsxwriter para colores y múltiples hojas
    with pd.ExcelWriter(out, engine="xlsxwriter") as writer:
        ...
        # Aplicar colores automáticos en el Excel
        ...
        # 2. Hoja de Balance
        df_balance_final.to_excel(writer, sheet_name="Balance_Operadores")
        
        # 3. Hoja de Cobertura
        df_check_final.to_excel(writer, sheet_name="Validacion_Cobertura")
