...
st.caption("Optimizado con exportación completa: Programación, Balance y Cumplimiento.")
...
# 4. MOTOR DE PROGRAMACIÓN (Optimizado para no colgarse)
@st.cache_data
def generar_programacion_nivelada(n_ops, d_req, n_req, d_semana, seed):
    ...
    for bloque_idx in [0, 21]:
        ...
        # Fase 2: Refuerzos Directos (Optimizado)
...
with c3:
    if st.button("👤 Asignar Fichas"):
        if "df" in st.session_state:
            ...
            st.success("Fichas asignadas.")

# 6. RENDERIZADO Y EXPORTACIÓN COMPLETA
if "df" in st.session_state:
    ...
    st.dataframe(df_visual.style.map(lambda v: f"background-color: {'#FFF3CD' if v=='D' else '#CCE5FF' if v=='N' else '#F8F9FA'}; font-weight: bold"), use_container_width=True)
    ...
    for idx in df_base.index:
        ...
        stats_list.append({
            ...
            "Horas Totales": (f!=DESCANSO).sum() * horas_turno,
            "Estado": "OK"
        })
    df_balance = pd.DataFrame(stats_list)

    # Preparar Datos de Cumplimiento
    cumplimiento = []
    for dia in NOMBRES_DIAS:
        ad, an = (df_base[dia] == TURNO_DIA).sum(), (df_base[dia] == TURNO_NOCHE).sum()
        cumplimiento.append({
            "Día": dia, "Día (Req)": demanda_dia, "Día (Asig)": ad,
            "Noche (Req)": demanda_noche, "Noche (Asig)": an, "Cumplimiento": "✅" if ad>=demanda_dia and an>=demanda_noche else "⚠️"
        })
    ...
    fill_hdr = PatternFill("solid", start_color="1F2937")
    ...
    for r_idx, row in enumerate(df_balance.values, 2):
        for c_idx, val in enumerate(row, 1): ws2.cell(r_idx, c_idx, val).border = border

    # HOJA 3: Cumplimiento (LO QUE PEDISTE)
    ws3 = wb.create_sheet("Cumplimiento")
    ws3.append(list(df_cumplimiento.columns))
    for cell in ws3[1]: cell.font = font_hdr; cell.fill = fill_hdr
    for r_idx, row in enumerate(df_cumplimiento.values, 2):
        for c_idx, val in enumerate(row, 1):
            cell = ws3.cell(r_idx, c_idx, val)
            cell.border = border; cell.alignment = align_c

    wb.save(out)
    st.download_button("⬇️ Descargar Excel Completo", out.getvalue(), f"Programacion_{cargo}.xlsx")
