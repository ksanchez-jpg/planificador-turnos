...
st.caption("Objetivo: 132h por ciclo (44h promedio), modelo 2x2 mixto y cobertura garantizada.")
...
# 4. MOTOR DE PROGRAMACIÓN CORREGIDO (GARANTÍA 132H)
def generar_programacion_equitativa(n_ops, d_req, n_req, d_semana):
    ...
    for inicio_bloque in [0, 21]:
        ...
        # Fase 1: Asignación por demanda operativa [cite: 4, 7]
        for d in range(inicio_bloque, fin_bloque):
            ...
            # Resto Día (Respetando descanso post-noche) [cite: 11, 13]
            ...
            for op in asignados_d:
                horario[op][d], turnos_bloque[op], racha[op] = TURNO_DIA, turnos_bloque[op] + 1, racha[op] + 1
        ...
        # Fase 2: BALANCEO OBLIGATORIO (Para llegar a 11 turnos/132h) [cite: 16]
        for op in ops:
            intentos = 0
            while turnos_bloque[op] < 11 and intentos < 500:
                ...
                # Reglas de inserción:
                # 1. Día laborable y actualmente en descanso
                if (d_rand % 7) >= d_semana or horario[op][d_rand] != DESCANSO: continue
                # 2. Respetar racha máxima de 2 [cite: 6]
                racha_prev = 0
                if d_rand > inicio_bloque and horario[op][d_rand-1] != DESCANSO: racha_prev += 1
                if d_rand > inicio_bloque + 1 and horario[op][d_rand-2] != DESCANSO and horario[op][d_rand-1] != DESCANSO: racha_prev += 1
                racha_post = 0
                if d_rand < fin_bloque - 1 and horario[op][d_rand+1] != DESCANSO: racha_post += 1
                
                if (racha_prev + racha_post + 1) > 2: continue
                # 3. Respetar descanso post-noche [cite: 11]
                if d_rand > inicio_bloque and horario[op][d_rand-1] == TURNO_NOCHE: continue
                
                # Insertar como Día para balancear horas
